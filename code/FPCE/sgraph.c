...
roaring_bitmap_t **rbmp = NULL;

void SGRAPH_init_bmp(SGRAPH *G)
{
  int i,j, nbr, k=0;
  rbmp = (roaring_bitmap_t **)malloc(sizeof(roaring_bitmap_t *) * G->edge.t);

  for(i = 0; i < G->edge.t; i++){
    rbmp[i] = NULL;
    if(G->edge.v[i].t > 0){
      rbmp[i] = roaring_bitmap_create_with_capacity(G->edge.v[i].t);
      if(rbmp[i] == NULL){
          print_err("Error - failed to allocate rbmp[%d]", i);
          exit(1);
      }
      for (j=0; j<G->edge.v[i].t ; j++){
        nbr = G->edge.v[i].v[j];
        roaring_bitmap_add(rbmp[i], nbr);
      }
    }//if current node has some nbrs
    //printf("\nInserted %lu keys in the hash table\n",roaring_bitmap_get_cardinality(rbmp[i]));fflush(stdout);
  }
  // check whether a value is contained
  //assert(roaring_bitmap_contains(rbmp[0], 1));
}


int SGRAPH_is_edge_bmp(int u, int v)
{
  return roaring_bitmap_contains(rbmp[u], v);
}


void SGRAPH_free_bmp(SGRAPH *G)
{
  unsigned int i;
  for(i = 0; i < G->edge.t; i++){
    if(rbmp[i] != NULL)  
      roaring_bitmap_free(rbmp[i]);
    rbmp[i] = NULL;
  }
  free(rbmp);
  rbmp = NULL;
  //print_err("cleared roaring bitmap ...");
}
...
void SGRAPH_init_hash(SGRAPH *G)
{
  int i,j, nbr, k=0;
  //AuthorA: since we aren't pushing any separate values (other than the key itself) into our hash
  //we can't provide a destroyer function for our values. Otherwise, a "double free error" occurs here
  edgesHash = g_hash_table_new_full( g_int64_hash, g_int64_equal, (GDestroyNotify)key_destroyed, NULL );

//  printf("%d size=%lu\n", sizeof(gint64), G->edge.eles/2);fflush(stdout);
  
  for(i = 0; i < G->edge.t; i++){
    for (j=0; j<G->edge.v[i].t ; j++){
      nbr = G->edge.v[i].v[j];
      if(i<nbr) { //AuthorA: to save space, only push (u,v) but not (v,u) where u<v      
        gint64 *hkey = g_new(gint64, 1);
        *hkey = SGRAPH_prep_edge(i, nbr);
//        printf("%lX\n", hkey);fflush(stdout);
//        gint64 tmp = SGRAPH_prep_edge(i, nbr);
//        char s[100] = "";
//        printf("%X-%X\n", i,nbr);fflush(stdout);
//        snprintf(s, 99, "%d-%d", i, nbr);
//        
//        printf("contains = %d\n", g_hash_table_contains(edgesHash,(gpointer) &tmp));
        g_hash_table_add(edgesHash, (gpointer) hkey );
//        printf("inserted = %d\n",rr);
//        printf("%s\n", s);fflush(stdout);
      }
    }
  }
  printf("\nInserted %d keys in the hash table\n",g_hash_table_size(edgesHash));fflush(stdout);
}

#define SHFT 32 //(8*sizeof(gint64))/2

//prepare edge to be inserted to our hash table
inline gint64 SGRAPH_prep_edge(int u, int v)
{
  //swap u and v if unequal to ensure that the value left-shifted is < the other value
  if(u > v){
    u ^= v; 
    v ^= u; 
    u ^= v;
  ...
  gint64 x = u;
  x = ( x << SHFT ) | v; //shift u 32 bits left and then OR with v

  return x;
}

int SGRAPH_is_edge(int u, int v)
{
//printf("check edge: %d-%d\n", u, v);fflush(stdout);
  gint64 hkey = SGRAPH_prep_edge(u,v);
//  printf("%lld - %d\n", hkey, g_hash_table_contains(edgesHash,&hkey));fflush(stdout);
  int res = (g_hash_table_contains(edgesHash,(gpointer) (&hkey)) )? 1 : 0;

//  g_free(hkey);
  return res;
}

void SGRAPH_free_hash()
{
  g_hash_table_destroy(edgesHash);
//  g_free(hkeys);
}
