...
SGRAPH INIT_SGRAPH3 = {TYPE_SGRAPH,NULL,0,INIT_SETFAMILY_,INIT_SETFAMILY_,INIT_SETFAMILY_,0,NULL,NULL,NULL};
//Author-A
...
void PROBLEM_init (PROBLEM *P){
  ...
  P->itemcand2 = P->itemjump = P->itemcand = P->vecjump = P->veccand = INIT_QUEUE;  // for delivery
...
void PROBLEM_alloc (PROBLEM *P, QUEUE_ID siz, QUEUE_ID siz2, size_t siz3, PERM *perm, int f){
  ...
  if ( f&PROBLEM_ITEMCAND ){ 
    QUEUE_alloc (&P->itemcand, siz+2);
    QUEUE_alloc (&P->itemcand2, siz+2);//AuthorA
  }
