...
extern int DONTEXTEND, maxNodes, returnLen;
...
#endif
{
  ...
  */
  minDegree = (int)(desiredDensity * (double)(minNodes-1));//not used if UNPROVEN_CONJECTURE is not defined
#endif
  ...
  */
  ...
  /// \todo check why they malloc(gintG->numNodes*sizeof(gintGraph *)) instead of malloc(gintG->numNodes*sizeof(gintNode *))?
  ...
  */
  ...
  /*
   Initialize list L[0] (==0th subgraph-chain, which assigned to the 0-th thread)
   with sorted dense subgraphs consisting of one edge.
   Also initialize a list of limbs (boundary nodes) for each subG in L[0].
  */
  masterCurr = L[0];
  for(i=0; i<gintG->numNodes; i++)
  {
    ...
    for(j=0; j<node->numEdges; j++)
    {
      ...
      if(node->edgeWeights[j] < desiredDensity) continue;	///Author-A, added this constraint so that it works for weighted graphs

      //create a subgraph chain (sgcPt) for each edge that have the min-density
      ...
      // add the pair of nodes of current edge into the 1st graph (subG here) of sgcPt subgraph-chain
      ...
      //store a sorted list of boundary nodes of current/last subgraph of the chain sgcPt (subG) in the lmbs data struct
      ...
      masterCurr->next = sgcPt;	//add the newly formed subgraph (consists of an edge) at the end of L[0]
      masterCurr       = sgcPt;	//update masterCurr so that it always points to the last entry in the L[0] subgraph-chain
    }//j
  }//i
  ...
  //now masterCurr contains a NULL subgraph (head), followed by a sorted list of 2-node subgraphs (edges in G), and then another NULL subgraph (tail)
...
/// \brief process the idx-th subgraph-chain/thread, specifically it takes the last subgraph in that chain (subG)
/// expands it (if possible) and check if the expanded graph is already in any L[.] list, if not we add it to
/// L[(idx+1)%NUM_THREADS] i.e. to the buffer of next thread to process it
int ProcessSubGraph(int idx, gintGraph *subG, int *limbs, int minNodes,
                    double desiredDensity)
{
  int i, j, k, canBeExtended, insertionIndex, u, logic;
  void *vp;
  double den, existingTotalEdgeWt, newTotalEdgeWt, oldDensity;
  register int tmp;
  gintNode *node;
  subGraphChain *sgcPt;
  ...
  /* count existing edge wt.s in subG */
  existingTotalEdgeWt = 0;
  for(j=0; j<subG->numNodes; j++)
  {
	  for(k=0; k<subG->nodeList[j]->numEdges; k++)
		  existingTotalEdgeWt += subG->nodeList[j]->edgeWeights[k];
  }

  existingTotalEdgeWt /= 2; /* each edge was counted twice */
  oldDensity = existingTotalEdgeWt / (double)(((subG->numNodes - 1) * (subG->numNodes))/2.0);

	/// Author-A: if you want fixed sized (of minNodes) clusters
	if(DONTEXTEND == 1)
	{
		//if yes, skip expansion of clusters with size == minNodes
		if(subG->numNodes >= minNodes && oldDensity >= desiredDensity)
		{
//			printf("Skipping expansion of a cluster of size: %d\n", subG->numNodes);
			goto CHECKING;
		}
	}
	else if(DONTEXTEND == 2)
	{
		//if yes, skip expansion of clusters with size == maxNodes
		if(subG->numNodes >= maxNodes && oldDensity >= desiredDensity)
		{
//			printf("Skipping expansion of a cluster of size: %d\n", subG->numNodes);
			goto CHECKING;
		}
  ...
  canBeExtended = 0;
  for(k=1; k<limbs[0]+1; k++)//foreach boundary node in limbs[1..limbs[0]]
  {
    /* find "node" in gintG whose label is limbs[k] */
    vp = (void *)(gintG->nodeList);
    vp = bsearch((void *)&(limbs[k]), vp, gintG->numNodes,
                 sizeof(gintNode *), compIntGintN);
    node = *(gintNode **)vp; /* node == the boundary node in the graph gintG for which node->label = limbs[k] */
    ...
    for(j = 0; j < node->numEdges; j++)
    {
      ...
      vp = bsearch((void *)&(node->edges[j]), vp, subG->numNodes, 
                   sizeof(gintNode *), compIntGintN);
      if(vp != NULL)//yes, there is a node in subG with which the boundary node "node" is connected, it is np
      {
    	  //Author-A: modified the density calculation for weighted graph
    	  gintNode *np = *(gintNode **)vp;
    	  //find the edge (node, np) via linear search
    	  for(u = 0; u < node->numEdges; u ++)
    	  {
    	  	if(node->edges[u] == np->label)
    	  	{
    	  		newTotalEdgeWt += node->edgeWeights[u];
    	  		break;
    	  	}
    	  }
      }
    ...
    */
    den = (double)(existingTotalEdgeWt + newTotalEdgeWt) /
          (double)(((subG->numNodes + 1) * (subG->numNodes))/2.0);

//    printf("Current cluster size: %d, existingTotalEdgeWt = %lf, newTotalEdgeWt = %lf, density = %lf\n", subG->numNodes, existingTotalEdgeWt,
//    		newTotalEdgeWt, den);

    //Author-A: added the extra condition for oldDensity \todo check whether this checking is required, if not why
    if(den >= desiredDensity)// && den <= oldDensity)
    {
      canBeExtended = 1;
      ...
      */
      ...
      // update hits[idx] to store the info that the expanded graph is in L[idx]
      // so that other threads can find this info
      lenL[idx]++;
      product[idx] = 1;
      ...
      //update subG (i.e. actually insert the new node "node" into it & update limbs accordingly
      ...
      sgcCurr[idx]       = sgcPt;
    }//if(den(subG+current boundary node "node") >= desiredDensity)
  }//foreach boundary node (of subG) in limbs[1..limbs[0]]

  CHECKING:
  if(DONTEXTEND == 1)
  {
	logic = (subG->numNodes == minNodes);
  }
  else if(DONTEXTEND == 2)
  {
	if(subG->numNodes == maxNodes)	//i.e. execution comes here via goto statement
		logic = 1;
	else
		logic = (!canBeExtended && subG->numNodes >= minNodes);//i.e. execution comes here via longer path (w/o goto statement)

//  	logic = (subG->numNodes >= minNodes);
//  	if(maxNodes == 0)
//  		logic = logic && !canBeExtended;
//  	else
//  		logic = logic && ((subG->numNodes < maxNodes && !canBeExtended) || subG->numNodes == maxNodes);
  }
  else
  {
	logic = (!canBeExtended && subG->numNodes >= minNodes);
  }
  /*
...
//    printf("Found a graph with %d nodes\n", subG->numNodes);
